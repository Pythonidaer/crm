...
function parsePatchTimestamp(value: unknown): string | null | undefined {
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    ...
    if (req.method === 'PATCH') {
      ...
      const nextFollowUpAt = parsePatchTimestamp(body['nextFollowUpAt'])
      const lastContactedAt = parsePatchTimestamp(body['lastContactedAt'])
      ...
      await sql.query(
        ...
        [
          ...
          nextFollowUpAt !== undefined
            ? nextFollowUpAt
            : toIsoOrNull(current.nextFollowUpAt),
          lastContactedAt !== undefined
            ? lastContactedAt
            : toIsoOrNull(current.lastContactedAt),
